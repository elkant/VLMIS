 ...
 * @author Emmanuel E
 */
public class importDSD extends HttpServlet {
...
System.out.println(" VL Data upload by "+user_id+" ");
           ...
           String checkexisting="select id from nonemr_diffcare where id  like '"+id+"' "; 
...
insert=" update nonemr_diffcare set ccc_number=?,Stability=?,DC_Model=?,well_unwell=?,user_id=?,timestamp=? "
     + " where id='"+id+"'  ";
...
conn.pst1.setString(2,Stability);
conn.pst1.setString(3,DC_Model);
conn.pst1.setString(4,well_unwell);
conn.pst1.setString(5,user_id);
  ...
  if(!id.contains("annual")){ //notify user only when importing weekly summaries
    txtresponse="<font color='green'> Data updated succesfully for patient "+ccc_number+"  </font> ";
                        ...
                        else 
                        {
                              ...
                              {
                        txtresponse="<font color='red'>Data </b></font><font color='red'> NOT updated </font><font color='red'> for patient  "+ccc_number+". because the record was updated by a super admin.</font>";
  ...
  if(!id.contains("annual")){
    ...
    if(id.contains("annual")){txtresponse="";}
        ...
        //send an email at this point of the exception
            ...
            try {
                sm.Sendemail("Facility DSD DATA IMPORT",ex.toString()+ "___ n user name: "+user_id+" n "+myresponse, "MYSQL IMPORTING ERROR ","EKaunda@fhi360.org");
